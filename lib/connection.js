...
issues.log = function( message ){
	...
	if( this.retries ){
		setTimeout( Utils.curry( issue, issue.attemptRetry ), this.retry_timeout );
...
Object.defineProperty( issues, 'details', {
	get: function(){
		...
		if( this.retries ){
			...
			res.totalDownTime = ( res.totalReconnectsFailed * this.reconnect ) + ( this.totalRetries * this.retry_timeout) ;
		}
...
Manager.free  = function( keep ){
	...
	while( this.connections.length ){
		...
		if( save.length < keep && this.isAvailable( this.connection[0] ) ){
			save.push( connection );
			continue;
		}
