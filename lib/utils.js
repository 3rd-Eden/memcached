    ...
    switch(tokens[1]){
      case Number:
        if (toString.call(value) !== '[object Number]') {
          err = new Error('Argument "' + key + '" is not a valid Number.');
        }
      ...
      case Boolean:
        if (toString.call(value) !== '[object Boolean]') {
          err = new Error('Argument "' + key + '" is not a valid Boolean.');
        }
      ...
      case Array:
        if (toString.call(value) !== '[object Array]') {
          err = new Error('Argument "' + key + '" is not a valid Array.');
        }
      ...
      case Object:
        if (toString.call(value) !== '[object Object]') {
          err = new Error('Argument "' + key + '" is not a valid Object.');
        }
      ...
      case Function:
        if (toString.call(value) !== '[object Function]') {
          err = new Error('Argument "' + key + '" is not a valid Function.');
        }
      ...
      case String:
        if (toString.call(value) !== '[object String]') {
          err = new Error('Argument "' + key + '" is not a valid String.');
        }
        ...
        if (!err && key === 'key') {
          if (value.length > config.maxKeySize) {
            if (config.keyCompression){
              ...
              err = new Error('Argument "' + key + '" is longer than the maximum allowed length of ' + config.maxKeySize);
            }
          } else if (/[\s\n\r]/.test(value)) {
            err = new Error('The key should not contain any whitespace or new lines');
          }
        }
        break;
      ...
      default:
        if (toString.call(value) === '[object global]' && !tokens[2]) {
          err = new Error('Argument "' + key + '" is not defined.');
        }
    }
  });
