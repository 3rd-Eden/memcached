...
exports.validate_arg = function validate_arg( args, config ){
		...
		switch( tokens[1] ){
			...
			case Function:
				if( toString.call( args[ tokens[0] ] ) !== '[object Function]' )
					err = 'Argument "' + tokens[0] + '" is not a valid Function.';
				break;
			...
			case String: 
				...
				if( !err && tokens[0] == 'key' && args[ tokens[0] ].length > config.max_key_size ){
					if( config.key_compression )
						args[ tokens[0] ] = CreateHash( 'md5' ).update( args[ tokens[0] ] ).digest( 'hex' );
					else
						err = 'Argument "' + tokens[0] + '" is longer than the maximum allowed length of ' + config.max_key_size;
				}
