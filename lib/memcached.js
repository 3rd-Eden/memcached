...
(function (nMemcached) {
  ...
  , 'VALUE': function value(tokens, dataSet, err, queue) {
      ...
      if (!multi) {
        if(this.metaData[0] && this.metaData[0].query_key && this.metaData[0].query_key <= this.maxKeySize) {
          // check for metadata
          if(this.metaData[0].query_key !== key) {
            err.push("Received key "+key+" did not match requested key "+this.metaData[0].key+"!");
