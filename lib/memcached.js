    ...
    while(S.bufferArray.length && privates.allCommands.test(S.bufferArray[0])) {
      ...
      if (privates.parsers[tokenSet[0]]) {
        ...
        if (tokenSet[0] === 'VALUE') {
          ...
          // Use Buffer.from().length to get the number of bytes instead of utf8 characters
          var lengthToGet = parseInt(tokenSet[3]),
            currentDataRead = Buffer.from(S.bufferArray[0], 'utf8').length;

          dataSet = Utils.unescapeValue(S.bufferArray.shift());

          while (currentDataRead < lengthToGet)
          {
            currentDataRead += Buffer.from(S.bufferArray[0], 'utf8').length + LINEBREAK.length;
            dataSet += LINEBREAK + Utils.unescapeValue(S.bufferArray.shift());
          }
        }
